...
for seed in seeds:
    ...
    # Create a dynamic folder name for this seed
    base_folder = f"data_class_params_const_test_seed_{seed}"
    os.makedirs(base_folder, exist_ok=True)
    
    # Output file for metrics (uses dynamic folder)
    metrics_file = open(f"{base_folder}/metrics_summary.txt", "w")
    ...
    for feature, hl in configs:
        print(f"\n{'-'*50}")
        print(f"Running Config: Feature Size={feature}, Quantum Output={feature}, HL={hl} (Seed: {seed})")
        print(f"{'-'*50}")
        ...
        np.savetxt(f"{base_folder}/training_history_F{feature}_HL{hl}.csv", hist, delimiter=",", header="Total,PDE,Consistency,Initial,Boundary", comments="")
        ...
        plt.savefig(f"{base_folder}/training_loss_F{feature}_HL{hl}.png", bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"{base_folder}/solutions_F{feature}_HL{hl}.png", bbox_inches='tight')
        plt.close()
        ...
        # Append RMSE to the tracking dictionary
    ...
    plt.ylabel("RMSE")
    plt.title(f"RMSE vs Quantum Parameter Ratio (Seed {seed})")
    plt.grid(True)
    ...
    plt.savefig(f"{base_folder}/rmse_vs_ratio.png", bbox_inches='tight')
    plt.close()
...
plt.title('Mean RMSE across Seeds with Standard Deviation (Classical Params Const Ratio)')
plt.xticks(x_pos, labels)
plt.grid(axis='y', linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig("aggregated_rmse_classical_params_const_ratio.png", dpi=300)
plt.show()

print("Aggregated plot saved as 'aggregated_rmse_classical_params_const_ratio.png'.")