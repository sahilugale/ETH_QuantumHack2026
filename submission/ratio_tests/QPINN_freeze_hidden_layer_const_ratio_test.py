...
for seed in seeds:
    ...
    base_folder = f"data_hidden_layer_const_frozen_test_seed_{seed}"
...
plt.title('Mean RMSE across Seeds with Standard Deviation\n(Hidden Layer Const - Frozen Quantum)')
plt.xticks(x_pos, labels)
plt.grid(axis='y', linestyle='--', alpha=0.7)

# Save the final aggregated plot in the current working directory
plt.tight_layout()
plt.savefig("aggregated_rmse_hidden_layer_const_ratio_frozen.png", dpi=300)
plt.show()

print("\nAggregated plot saved as 'aggregated_rmse_hidden_layer_const_ratio_frozen.png'.")